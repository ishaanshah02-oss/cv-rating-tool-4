...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!fileInput || !rateBtn || !cvText || !results) {
    alert('Missing elements on the page. Check index.html IDs.');
    return;
  }

  fileInput.addEventListener('change', handleFile);
  rateBtn.addEventListener('click', rateCV);
  ...
  async function extractPdfText(file) {
    if (!window.pdfjsLib) {
      alert('PDF.js failed to load. Paste your CV text instead.');
      return;
    }
